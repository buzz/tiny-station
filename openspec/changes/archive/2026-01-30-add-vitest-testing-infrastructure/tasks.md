...
- [x] Install root-level devDependencies for Vitest
  - `vitest`
  - `vite-tsconfig-paths`
  - Validate with `pnpm list --depth -1` in root

- [x] Create root-level `vitest.config.ts` for workspace patterns
...
- [x] Install devDependencies in `packages/common`
  - `vitest`
  - `vite-tsconfig-paths`
...
- [x] Write sample schema test in `packages/common/src/apiSchemas.test.ts`
...
- [x] Install devDependencies in `packages/server`
  - `vitest`
  - `vite-tsconfig-paths`
...
- [x] Add test scripts to `packages/server/package.json`
...
- [x] Write unit test example in `packages/server/src/plugins/api/AuthService.test.ts`
  - Mock Redis dependency
  - Test service logic in isolation
  - Run tests to verify they pass

- [x] Write integration test example in `packages/server/src/plugins/api/apiRoutes.integration.test.ts`
  - Create temporary Fastify instance
  - Test route behavior
  - Mock Socket.IO and Redis
  - Run tests to verify they pass

- [x] Create mock patterns in `packages/server/src/__mocks__/`
...
- [x] Install devDependencies in `packages/frontend`
  - `vitest`
  - `vite-tsconfig-paths`
...
- [x] Add test scripts to `packages/frontend/package.json`
  - `test` - runs vitest
  - `test:watch` - runs vitest with watch mode
  - Validate with `cd packages/frontend && pnpm run test --run`

- [x] Write component test example in `packages/frontend/src/components/Modal/Modal.test.tsx`
  - Test component rendering
  - Test props and state
  - Test with @testing-library/react
  - Run tests to verify they pass

- [x] Write component test with user interaction in `packages/frontend/src/components/NavBar/VolumeControl.test.tsx`
...
- [x] Update `AGENTS.md` with testing commands
